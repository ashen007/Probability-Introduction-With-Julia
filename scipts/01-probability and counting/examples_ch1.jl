...
md"""
A coin is flipped 10 times. Heads as _H_ and Tails as _T_, a possible outcome is _HHHTHHTTHT_, and the sample space is the set of all possible strings of length 10 of _H_'s and _T_'s. we can encode _H_ as 1 and _T_ as 0, so that outcome is a sequence $(s_1, ..., s_10)$ with $s_j \in \lbrace 0, 1 \rbrace$, and the sample space is the set of all such sequences. 
"""
...
$$A_1 = \lbrace (1, s_2, ..., s_10): s_j \in \lbrace 0, 1 \rbrace \text{ for } 2 \leq j \leq 10 \rbrace$$
"""

# ╔═╡ e370f6fc-38ec-4af0-a1b9-3755e6b51fe4
md"""
...
# ╔═╡ 41e81641-93fe-43a3-a2d2-43e9981e85e3
md"""
- let B be the event that at least one flip was Heads. as a set,

$$B = {\bigcup}_{j=1}^{10} A_j$$
"""

# ╔═╡ e90b19bb-3f8a-4f47-859f-21710d0deb37
md"""
- let C be the event that all the flips were Heads. as a set,

$$C = {\bigcap}_{j=1}^{10} A_j$$
"""

# ╔═╡ c5a32c87-25f2-4e62-bd26-8f2d3e04f5c5
md"""
- let D be the event that there were at least two consecutive Heads. as a set,

$$D = {\bigcup}_{j=1}^9 (A_j \cap A_{j+1})$$
"""

# ╔═╡ d4576a59-0ebf-473f-92d7-a45f801a6bd5
md"""
## 1.2.3 pick a any card
"""

# ╔═╡ b67c6fa2-e2f6-41ed-8398-10d4026ad045
md"""
...
# ╔═╡ efba55bc-2aac-4fb1-88d4-8f9d406c5a92
md"""
## 1.4.3 runners
"""

# ╔═╡ a663812a-0b47-4de2-88a4-4a617bef2e23
md"""
...
# ╔═╡ 343cc245-25d8-42ee-bc1a-12e9dcad729c
begin
first = 10
second = 10 - 1
third = 10 - 1 - 1

all_poss_outcome = first * second * third
end

# ╔═╡ 921021f9-482d-44a8-ae0d-c69e3e7ee6a1
md"""
## 1.4.5 ice cream cones
"""

# ╔═╡ baa483cb-e508-4a12-affc-164e602d8af9
md"""
By the multiplication rule, there are $2 . 3 = 6$ possibilities. This is a very simple
example, but is worth thinking through in detail as a foundation for thinking about
and visualizing more complicated examples.

1. It doesn't matter whether you choose the type of cone first ("I'd like a waffle
   cone with chocolate ice cream") or the avor first ("I'd like chocolate ice cream      on a waffle cone"). Either way, there are $2 . 3 = 3 . 2 = 6$ possibilities.

2. It doesn't matter whether the same flavors are available on a cake cone as on a
   waffle cone. What matters is that there are exactly 3 flavor choices for each cone
   choice. If for some strange reason it were forbidden to have chocolate ice cream      on a waffle cone, with no substitute favor available (aside from vanilla and          strawberry), there would be $3 + 2 = 5$ possibilities and the multiplication rule     wouldn't apply.
"""

# ╔═╡ 1d03e16a-0c62-4649-9ac3-0148c84f06a6
md"""
...
# ╔═╡ 4673bc99-79ce-4e3b-ad10-aa6d7b919948
md"""
But what if you're only interested in what kinds of ice cream cones you had that
day, not the order in which you had them, so you don't want to distinguish, for
example, between $(cakeC, waffleV)$ and $(waffleV, cakeC)$? Are there now $36 / 2 = 18$ possibilities? No, since possibilities like $(cakeC, cakeC)$ were already only listed once each. There are $6 . 5 = 30$ ordered possibilities $(x, y)$ with $x \neq y$, which turn into 15 possibilities if we treat $(x, y)$ as equivalent to $(y, x)$, plus 6 possibilities of the form $(x, x)$, giving a total of 21 possibilities. Note that if the 36 original ordered pairs $(x, y)$ are equally likely, then the 21 possibilities here are not equally likely.
"""

# ╔═╡ 45e0cf4d-e1da-44ff-bebc-00f79f3324bf
md"""
## 1.4.6 subsets
"""

# ╔═╡ a8b14924-eb8d-4d35-a5ff-9c3bebdceafc
md"""
A set with n elements has $2^n$ subsets, including the empty set $\emptyset$ and the set itself. This follows from the multiplication rule since for each element, we can choose whether to include it or exclude it.

$$\lbrace 1, 2, 3 \rbrace = \emptyset, \lbrace 1 \rbrace, \lbrace 2 \rbrace, \lbrace 3 \rbrace, \lbrace 1, 2 \rbrace, \lbrace 1, 3 \rbrace, \lbrace 2, 3 \rbrace, \lbrace 1, 2, 3 \rbrace$$
"""

# ╔═╡ fce78d2a-ce6d-4641-bd20-66f9d7aae295
md"""
## 1.4.10 birthday problem
"""

# ╔═╡ 210a5c9a-c291-4a4e-91ef-f684f47d3d53
md"""
There are $k$ people in a room. Assume each person's birthday is equally likely to be any of the 365 days of the year (we exclude February 29), and that people's birthdays are independent. What is the probability that at least one pair of people in the group have the same birthday?
"""

# ╔═╡ d8df2982-ad4d-4d78-bb90-9923ea86a5d3
function sampling_days_without_replace(k)
	if k <= 365
		days = [BigInt(365 - i + 1) for i=1:k]
		prod(days)
...
begin
k = 23
no_matching_bdays = sampling_days_without_replace(k) / BigInt(365)^k
matching_days = 1 - no_matching_bdays
end

# ╔═╡ 3cbf9695-292f-40ff-beab-85091e642a20
begin
ks = 100
ps = [(1 - sampling_days_without_replace(i) / BigInt(365)^i) for i=0:ks]
